layui.define(['jquery','layer'], function (exports) {
    //来源于layui第三方组件，已经修改，没有数据时列头还是会显示，
    ...
    tree.prototype.template = function (e) {
        ...
        o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function () {
            ...
            var tr = o(this).parents('[data-id]'),
                id = tr.data('id'),
                pid = tr.data('pid'),
                filter = o(this).attr("lay-event");
            return layui.event.call(this, MOD_NAME, 'tree(' + e.elem.replace('#', '') + ')', {
                tr: tr,
                elem: this,
                event: filter,
    ...
    tree.prototype.template_tr = function (e, is_table, checked, checkbox, arr) {
        var html='';
        if (arr.length > 0) {
            var t = this;
            o.each(arr, function (idx1, item) {
                var tr = '',
                    is_checked = false,
                    hide_class = item.level <=1 ? '' : e.hide_class;
                item.is_end = !e.childs[item[e.primary_key]];
                o.each(e.cols, function (index, obj) {
                    var style = '';
                    obj.width && (style += 'width:' + obj.width + 'px;'), obj.align && (style += 'text-align:' + obj.align + ';'), style && (style = 'style="' + style + '"');
                    // 标记设置行checkbox选中
                    ...
                    // 指定列加入开启、关闭小图标t.cache(e, item[e.primary_key])
                    var icon = (obj.field == e.icon_key && !item.is_end) ? '<i class="' + (item.level<1 ? e.icon.open : e.icon.close) + '"></i>' : '<span></span>';
                    // 指定列小图标按照层级向后位移
                    var left = (obj.field == e.icon_key ? item.level * e.icon.left + 'px' : '');
                    icon = icon.replace('>', ' style="margin-left:' + left + ';">');
                    // 拼接行
                    tr += '<td ' + style + (left ? 'data-down' : '') + '>' + icon + (is_table ? '' : (is_checked ? checked : checkbox)) + (obj.templet ? obj.templet(item) : item[obj.field]) + '</td>';
                });
                var box = is_table ? o(is_checked ? checked : checkbox).wrap('<td style="width:28px;">').parent().prop('outerHTML') : '';
                html += '<tr class="' + hide_class + '" data-id="' + item[e.primary_key] + '" data-pid="' + item[e.parent_key] + '">' + box + tr + '</tr>';
                html+=t.template_tr(e, is_table, checked, checkbox,item.tempP);
            });
        }
        return html;
    };
    ...
    tree.prototype.data = function (e) {
        ...
        e.childs = childs;
        var tempdata = this.tree_data(e, lists, e.top_value, []);
        return tempdata;
    };
    tree.prototype.tree_data = function (e, lists, pid, data) {
        ...
        o.each(e.data, function (idx, item) {
            item.tempP = [];
            item.level = 0;
            map.set(item[e.primary_key],item);
        });
        map.forEach(function (val) {
            var m = map.get(val[e.parent_key]);
            if (m != null) {
                val.level = m.level + 1;
                m.tempP.push(val);
                o.each(val.tempP, function (i, item1) {
                    item1.level = val.level + 1;
                });
            } else {
                data.push(val);
            }
    ...
    tree.prototype.render = function (e) {
        ...
        if (e.url) {
            o.ajax({
                url: e.url,
                data:JSON.stringify(e.where),
                type: e.method == 'post' ? 'post' : 'get',
                dataType: 'json',
    ...
    tree.prototype.checked = function (e) {
        ...
        var ddd=tempData.filter(c => ids.indexOf(c.Id) > -1);
        return ids;
    };
    // 全部展开
